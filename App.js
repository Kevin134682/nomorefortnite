        ...
        <div className="container mx-auto px-6 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <div className="w-10 h-10 bg-purple-600 rounded-xl flex items-center justify-center glow-purple">
              <span className="font-arcane text-2xl text-white">A</span>
            </div>
          ...
          <div className="hidden md:flex flex-1 max-w-md mx-8">
            <div className="relative w-full">
              <input 
                ...
                className="w-full bg-slate-900/50 border border-slate-700/50 rounded-full py-2.5 px-12 focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500/50 transition-all text-sm text-white"
              />
              <svg className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
            </div>
          </div>
          <div className="flex items-center space-x-4">
             <button className="hidden sm:block px-5 py-2 glass rounded-full text-sm font-semibold hover:bg-purple-600 transition-all duration-300 text-white">
              Join Discord
            </button>
          </div>
        </div>
      ...
      <main className="container mx-auto px-6 pt-32">
        <div className="md:hidden mb-8">
          <input 
            ...
            value=${searchQuery}
            onInput=${(e) => setSearchQuery(e.target.value)}
            className="w-full bg-slate-900 border border-slate-700 rounded-xl py-3 px-6 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all text-white"
          />
        </div>

        ${selectedCategory === 'All' && !searchQuery && hotGames.length > 0 && html`
          <section className="mb-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ${hotGames.map(game => html`<${GameCard} key=${`hot-${game.id}`} game=${game} onClick=${setActiveGame} />`)}
            </div>
          </section>
        `}
          ...
          ` : html`
            <div className="flex flex-col items-center justify-center py-20 text-center">
              <div className="w-20 h-20 bg-slate-900 rounded-full flex items-center justify-center mb-4 border border-slate-800">
                <svg className="text-slate-600" width="32" height="32" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <footer className="mt-20 py-12 glass border-t-0">
        <div className="container mx-auto px-6 flex flex-col md:flex-row items-center justify-between">
          <div className="flex items-center space-x-2 mb-6 md:mb-0">
            ...
            <span className="text-slate-600 text-sm ml-2">© 2024 Arcane Entertainment</span>
          </div>
